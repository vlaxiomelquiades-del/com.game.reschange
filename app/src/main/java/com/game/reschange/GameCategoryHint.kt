...
object GameCategoryHint {
    ...
    /**
     * BUG FIX: se este app foi forcado por nos antes, mas o sistema nao
     * reconhece mais como jogo AGORA (a categoria reverteu sozinha --
     * ver comentario no passo 3 de runCategoryChange), refaz a forcagem
     * silenciosamente. Chame isso ANTES de aplicar/resetar a resolucao:
     * sem isso, `cmd game mode <mode> $pkg` vira no-op silencioso pro
     * GameManagerService (que so aceita pacotes que ele considera jogo),
     * mas o app continua mostrando Toast de sucesso porque nunca checava
     * a saida do comando. E exatamente o "nao consigo mudar nem resetar"
     * -- a resolucao para de responder e nao ha erro nenhum na tela.
     */
    fun reforceIfNeeded(context: Context, pkg: String): Result? {
        if (!isForcedByUs(context, pkg)) return null
        if (isActuallyGame(context, pkg)) return null
        return forceGameCategory(context, pkg)
    }
    ...
    private fun runCategoryChange(context: Context, pkg: String, targetCategory: Int): Result {
        ...
        // 3) BUG FIX: restaura o installer original E RE-VERIFICA depois.
        // Suspeita principal do bug relatado: em alguns firmwares, um
        // reinstall completo (mesmo -r/keep-data) parece rederivar o
        // ApplicationInfo a partir do manifest outra vez, apagando o
        // categoryHint que acabamos de setar no passo 2 -- mesmo com a
        // categoria corretamente VERIFICADA antes de restaurar. Sem essa
        // reverificacao, este metodo retornava sucesso mesmo quando a
        // categoria ja tinha revertido silenciosamente por baixo do pano,
        // e so aparecia depois como "aplicar resolucao nao faz nada".
        if (originalInstaller.isNullOrBlank() || originalInstaller == SHELL_PKG) {
            return Result(true, workedCode, "Categoria de jogo $label e VERIFICADA (código $workedCode, category=$targetCategory confirmado)")
        }

        reinstallWithInstaller(context, pkg, originalInstaller)
        if (readActualCategory(context, pkg) == targetCategory) {
            return Result(true, workedCode, "Categoria de jogo $label e VERIFICADA (código $workedCode, category=$targetCategory confirmado)")
        }

        // A restauracao do installer desfez a categoria. Categoria > installer
        // aqui -- refaz sem restaurar de novo, e deixa o installer em
        // com.android.shell (a alternativa e a categoria nao pegar de
        // jeito nenhum e a resolucao nunca aplicar, o que e pior).
        reinstallWithInstaller(context, pkg, SHELL_PKG)
        runServiceCall(context, workedCode, pkg, targetCategory)
        return if (readActualCategory(context, pkg) == targetCategory) {
            Result(true, workedCode,
                "Categoria de jogo $label (código $workedCode). Restaurar o installer original " +
                    "($originalInstaller) reverte a categoria neste firmware -- o installer deste " +
                    "app ficará com.android.shell para a mudança se manter.")
        } else {
            Result(false, workedCode,
                "Categoria mudou e foi verificada, mas reverteu ao restaurar o installer original, " +
                    "e não colou de novo na segunda tentativa. Código usado: $workedCode.")
        }
    }
